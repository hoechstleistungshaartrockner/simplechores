...
class SimpleChoresOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_add_member_assign_chores(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
    ...
    async def async_step_add_member_assignment_mode(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
                ...
                if chore:
                    ...
                    if assignment_mode == ASSIGN_MODE_ALWAYS:
                        ...
                        if len(chore.possible_assignees) >= 2:
                            ...
                            if (
                                not chore.assigned_to
                                or chore.assigned_to not in chore.possible_assignees
                            ):
                                chore.assigned_to = chore.possible_assignees[0]
                        else:
                    ...
                    for chore_id, chore in chores.items():
                        ...
                        if self._selected_member in chore.possible_assignees:
                            chore.possible_assignees = [
                                new_name
                                if assignee == self._selected_member
                                else assignee
    ...
    async def async_step_chore_basic(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
    ...
    async def async_step_chore_assignees(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        ...
        if user_input is not None:
            ...
            if assignment_mode == ASSIGN_MODE_ALWAYS and len(assignees) != 1:
                ...
                assignment_mode in [ASSIGN_MODE_ROTATE, ASSIGN_MODE_RANDOM]
                and len(assignees) < 2
            ):
        ...
        schema = vol.Schema(
            {
                ...
                vol.Required(
                    "assignment_mode", default=default_assignment_mode
                ): vol.In(
                    {
                        ASSIGN_MODE_ALWAYS: "Always (same person)",
                        ASSIGN_MODE_ROTATE: "Rotate (take turns)",
        ...
        step_id = (
            "edit_chore_assignees"
            if self._chore_mode == "edit"
            else "add_chore_assignees"
        )
        return self.async_show_form(
            step_id=step_id,
            data_schema=schema,
    ...
    async def async_step_chore_recurrence(
        ...
        if user_input is not None:
            recurrence_pattern = user_input.get(
                CONF_RECURRENCE_PATTERN, FREQUENCY_DAILY
            )
        ...
        step_id = (
            "edit_chore_recurrence"
            if self._chore_mode == "edit"
            else "add_chore_recurrence"
        )
        return self.async_show_form(
            step_id=step_id,
            data_schema=schema,
    ...
    async def async_step_chore_interval(
        ...
        if user_input is not None:
            self._chore_data[CONF_RECURRENCE_INTERVAL] = user_input.get(
                CONF_RECURRENCE_INTERVAL, 1
            )
            return await self.async_step_chore_finalize()
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    CONF_RECURRENCE_INTERVAL, default=default_interval
    ...
    async def async_step_chore_monthly_day(
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    CONF_RECURRENCE_DAY_OF_MONTH, default=default_day
        ...
        step_id = (
            "edit_chore_monthly_day"
            if self._chore_mode == "edit"
            else "add_chore_monthly_day"
        )
        return self.async_show_form(
            step_id=step_id,
            data_schema=schema,
    ...
    async def async_step_chore_monthly_weekday(
        ...
        if self._chore_mode == "edit" and self._selected_chore:
            ...
            if self._selected_chore in chores:
                ...
                if chore.recurrence_specific_weekdays:
                    default_weekdays = [
                        str(day) for day in chore.recurrence_specific_weekdays
                    ]
        ...
        if user_input is not None:
            ...
            if not errors:
                self._chore_data[CONF_RECURRENCE_WEEK_OF_MONTH] = [
                    int(week) for week in selected_weeks
                ]
                self._chore_data[CONF_RECURRENCE_SPECIFIC_WEEKDAYS] = [
                    int(day) for day in selected_weekdays
                ]
                return await self.async_step_chore_finalize()
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    CONF_RECURRENCE_WEEK_OF_MONTH, default=default_weeks
                ): cv.multi_select(
                    {
                        "1": "First",
                        "2": "Second",
        ...
        step_id = (
            "edit_chore_monthly_weekday"
            if self._chore_mode == "edit"
            else "add_chore_monthly_weekday"
        )
        return self.async_show_form(
            step_id=step_id,
            data_schema=schema,
    ...
    async def async_step_chore_specific_days(
        ...
        if self._chore_mode == "edit" and self._selected_chore:
            ...
            if self._selected_chore in chores:
                chore = chores[self._selected_chore]
                default_weekdays = [
                    str(day) for day in (chore.recurrence_specific_weekdays or [])
                ]
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    CONF_RECURRENCE_SPECIFIC_WEEKDAYS, default=default_weekdays
                ): cv.multi_select(
                    {
                        "0": "Monday",
                        "1": "Tuesday",
        ...
        step_id = (
            "edit_chore_specific_days"
            if self._chore_mode == "edit"
            else "add_chore_specific_days"
        )
        return self.async_show_form(
            step_id=step_id,
            data_schema=schema,
    ...
    async def async_step_chore_annual(
        ...
        schema = vol.Schema(
            {
                vol.Required(
                    CONF_RECURRENCE_ANNUAL_MONTH, default=default_month
                ): vol.In(
                    {
                        1: "January",
                        2: "February",
                ...
                vol.Required(
                    CONF_RECURRENCE_ANNUAL_DAY, default=default_day
                ): selector.NumberSelector(
                    selector.NumberSelectorConfig(
                        min=1,
                        max=31,
        ...
        step_id = (
            "edit_chore_annual" if self._chore_mode == "edit" else "add_chore_annual"
        )
        return self.async_show_form(
            step_id=step_id,
            data_schema=schema,
    ...
    async def async_step_chore_finalize(
        ...
        if self._chore_mode == "add":
            ...
            chore = Chore(
                ...
                assignment_mode=self._chore_data.get(
                    "assignment_mode", ASSIGN_MODE_ALWAYS
                ),
                possible_assignees=self._chore_data.get(
                    "assignees", list(members.keys())
                ),
                recurrence_pattern=self._chore_data.get(
                    CONF_RECURRENCE_PATTERN, FREQUENCY_DAILY
                ),
                recurrence_interval=self._chore_data.get(CONF_RECURRENCE_INTERVAL, 1),
                recurrence_day_of_month=self._chore_data.get(
                    CONF_RECURRENCE_DAY_OF_MONTH
                ),
                recurrence_week_of_month=self._chore_data.get(
                    CONF_RECURRENCE_WEEK_OF_MONTH
                ),
                recurrence_specific_weekdays=self._chore_data.get(
                    CONF_RECURRENCE_SPECIFIC_WEEKDAYS, []
                ),
                recurrence_annual_month=self._chore_data.get(
                    CONF_RECURRENCE_ANNUAL_MONTH
                ),
            ...
            chore.assignment_mode = self._chore_data.get(
                "assignment_mode", ASSIGN_MODE_ALWAYS
            )
            chore.possible_assignees = self._chore_data.get("assignees", [])
            chore.recurrence_pattern = self._chore_data.get(
                CONF_RECURRENCE_PATTERN, FREQUENCY_DAILY
            )
            chore.recurrence_interval = self._chore_data.get(
                CONF_RECURRENCE_INTERVAL, 1
            )
            chore.recurrence_day_of_month = self._chore_data.get(
                CONF_RECURRENCE_DAY_OF_MONTH
            )
            chore.recurrence_week_of_month = self._chore_data.get(
                CONF_RECURRENCE_WEEK_OF_MONTH
            )
            chore.recurrence_specific_weekdays = self._chore_data.get(
                CONF_RECURRENCE_SPECIFIC_WEEKDAYS, []
            )
            chore.recurrence_annual_month = self._chore_data.get(
                CONF_RECURRENCE_ANNUAL_MONTH
            )
            chore.recurrence_annual_day = self._chore_data.get(
                CONF_RECURRENCE_ANNUAL_DAY
            )
    ...
    async def async_step_edit_chore(
        ...
        chore_choices = {
            chore_id: f"{chore.name} ({area_id_to_name.get(chore.area_id, 'Unknown')})"
            for chore_id, chore in chores.items()
        }
        chore_choices = dict(
            sorted(chore_choices.items(), key=lambda item: item[1])
        )  # Sort by chore name
    ...
    async def async_step_edit_chore_basic(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        """Edit chore - basic info."""
        return await self.async_step_chore_basic(user_input)

    async def async_step_edit_chore_assignees(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        """Edit chore - assignees."""
        return await self.async_step_chore_assignees(user_input)

    async def async_step_edit_chore_recurrence(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        """Edit chore - recurrence."""
        return await self.async_step_chore_recurrence(user_input)

    async def async_step_edit_chore_interval(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        """Edit chore - interval."""
        return await self.async_step_chore_interval(user_input)

    async def async_step_edit_chore_monthly_day(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        """Edit chore - monthly day."""
        return await self.async_step_chore_monthly_day(user_input)

    async def async_step_edit_chore_monthly_weekday(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        """Edit chore - monthly weekday."""
        return await self.async_step_chore_monthly_weekday(user_input)

    async def async_step_edit_chore_specific_days(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        """Edit chore - specific days."""
        return await self.async_step_chore_specific_days(user_input)

    async def async_step_edit_chore_annual(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        """Edit chore - annual."""
        return await self.async_step_chore_annual(user_input)
    ...
    async def async_step_delete_all_chores(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
    ...
    async def async_step_delete_member(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        ...
        if user_input is not None:
            ...
            has_assigned_chores = any(
                chore.assigned_to == self._selected_member
                or self._selected_member in chore.possible_assignees
    ...
    async def async_step_delete_member_reassign(
        self, user_input: Optional[dict[str, Any]] = None
    ) -> FlowResult:
        ...
        remaining_members = {
            k: v for k, v in members.items() if k != self._selected_member
        }
    ...
    async def _finalize_delete_member(self, reassign_to: str | None) -> FlowResult:
        ...
        for chore_id, chore in chores.items():
            ...
            if len(chore.possible_assignees) == 1 and chore.assignment_mode in [
                ASSIGN_MODE_ROTATE,
                ASSIGN_MODE_RANDOM,
            ]:
                chore.assignment_mode = ASSIGN_MODE_ALWAYS
